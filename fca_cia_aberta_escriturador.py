"""
Script: fca_cia_aberta_escriturador.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2026\fca_cia_aberta_escriturador_2026.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_escriturador.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "CNPJ_Escriturador": "cnpj_escriturador",
    ...
    "Fax":               "fax",
    "Email":             "email",
    "Data_Inicio_Atuacao": "data_inicio_atuacao",
    "Data_Fim_Atuacao":  "data_fim_atuacao",
}
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Datas
    for col in ["data_referencia", "data_inicio_atuacao", "data_fim_atuacao"]:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "escriturador", "cnpj_escriturador",
        "tipo_endereco", "logradouro", "complemento", "bairro", "cidade",
        "sigla_uf", "pais", "cep", "ddi_telefone", "ddd_telefone", "telefone",
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_escriturador ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_escriturador WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_escriturador WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_escriturador ===")
