...
WITH ipo_base AS (
    ...
    JOIN cvm_data.cad_valor_mobiliario    v
        ON  v.cnpj_companhia = o.cnpj_emissor
...
SELECT
    ...
    ROUND(((c.adj_close - p.preco_oferta) / p.preco_oferta)::NUMERIC, 6) AS first_day_return,
...
COMMENT ON VIEW cvm_data.vw_first_day_return IS
    'First-Day Return: (adj_close_D1 - preco_oferta) / preco_oferta. Requer cotacoes_diarias carregado.';
...
COMMENT ON VIEW cvm_data.vw_avg_first_day_return IS
    'Avg. First-Day Return: media dos FDR dos IPOs nos 60 dias anteriores a cada IPO.';
    ...
    SELECT
        ...
        (
            SELECT EXP(SUM(LN(NULLIF(
                       c2.adj_close /
                       NULLIF(LAG(c2.adj_close) OVER (PARTITION BY c2.ticker ORDER BY c2.data), 0)
                   , 0)))) - 1.0
...
COMMENT ON VIEW cvm_data.vw_post_ipo_return IS
    'Post-IPO Return (BHAR) em janelas 3m/6m/1a. BHAR = retorno_acao - retorno_mercado (Rm NEFIN).';
