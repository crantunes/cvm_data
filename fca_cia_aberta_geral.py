"""
Script: fca_cia_aberta_geral.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2026\fca_cia_aberta_geral_2026.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_geral.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "Data_Nome_Empresarial":             "data_nome_empresarial",
    "Nome_Empresarial_Anterior":         "nome_empresarial_anterior",
    "Data_Constituicao":                 "data_constituicao",
    "Codigo_CVM":                        "codigo_cvm",
    "Data_Registro_CVM":                 "data_registro_cvm",
    "Categoria_Registro_CVM":            "categoria_registro_cvm",
    "Data_Categoria_Registro_CVM":       "data_categoria_registro_cvm",
    "Situacao_Registro_CVM":             "situacao_registro_cvm",
    "Data_Situacao_Registro_CVM":        "data_situacao_registro_cvm",
    "Pais_Origem":                       "pais_origem",
    "Pais_Custodia_Valores_Mobiliarios": "pais_custodia_valores_mobiliarios",
    ...
    "Especie_Controle_Acionario":        "especie_controle_acionario",
    "Data_Especie_Controle_Acionario":   "data_especie_controle_acionario",
    "Dia_Encerramento_Exercicio_Social": "dia_encerramento_exercicio_social",
    "Mes_Encerramento_Exercicio_Social": "mes_encerramento_exercicio_social",
    "Data_Alteracao_Exercicio_Social":   "data_alteracao_exercicio_social",
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["dia_encerramento_exercicio_social"]  = pd.to_numeric(df["dia_encerramento_exercicio_social"], errors="coerce").astype("Int16")
    df["mes_encerramento_exercicio_social"]  = pd.to_numeric(df["mes_encerramento_exercicio_social"], errors="coerce").astype("Int16")

    # Strings — remove espaços extras e substitui string vazia por None
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "nome_empresarial_anterior",
        "codigo_cvm", "categoria_registro_cvm", "situacao_registro_cvm",
        "pais_origem", "pais_custodia_valores_mobiliarios", "setor_atividade",
        "descricao_atividade", "situacao_emissor", "especie_controle_acionario",
        "pagina_web",
    ]
    for col in str_cols:
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_geral ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_geral WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_geral WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_geral ===")
