...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2027\fca_cia_aberta_dri_2027.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_dri.log", encoding="utf-8"),
    ],
)
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "tipo_responsavel", "responsavel",
        "cpf_responsavel", "tipo_endereco", "logradouro", "complemento",
        "bairro", "cidade", "sigla_uf", "uf", "pais", "cep",
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_dri ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_dri WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_dri WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_dri ===")
