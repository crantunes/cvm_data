"""
Script: fca_cia_aberta_valor_mobiliario.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2026\fca_cia_aberta_valor_mobiliario_2026.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_valor_mobiliario.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "Sigla_Classe_Acao_Preferencial": "sigla_classe_acao_preferencial",
    "Classe_Acao_Preferencial":       "classe_acao_preferencial",
    "Codigo_Negociacao":              "codigo_negociacao",
    "Composicao_BDR_Unit":            "composicao_bdr_unit",
    "Mercado":                        "mercado",
    "Sigla_Entidade_Administradora":  "sigla_entidade_administradora",
    "Entidade_Administradora":        "entidade_administradora",
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "valor_mobiliario",
        "sigla_classe_acao_preferencial", "classe_acao_preferencial",
        "codigo_negociacao", "composicao_bdr_unit", "mercado",
        "sigla_entidade_administradora", "entidade_administradora", "segmento",
    ]
    for col in str_cols:
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_valor_mobiliario ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_valor_mobiliario WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_valor_mobiliario WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_valor_mobiliario ===")
