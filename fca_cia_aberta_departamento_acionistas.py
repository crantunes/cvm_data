"""
Script: fca_cia_aberta_departamento_acionistas.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2024\fca_cia_aberta_departamento_acionistas_2024.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_departamento_acionistas.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "Contato":           "contato",
    "Data_Inicio_Contato": "data_inicio_contato",
    "Data_Fim_Contato":  "data_fim_contato",
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Datas
    for col in ["data_referencia", "data_inicio_contato", "data_fim_contato"]:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "contato", "tipo_endereco",
        "logradouro", "complemento", "bairro", "cidade", "sigla_uf", "pais",
        "cep", "ddi_telefone", "ddd_telefone", "telefone",
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_departamento_acionistas ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_departamento_acionistas WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_departamento_acionistas WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_departamento_acionistas ===")
