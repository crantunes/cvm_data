"""
Script: fca_cia_aberta_canal_divulgacao.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2027\fca_cia_aberta_canal_divulgacao_2027.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_canal_divulgacao.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "Canal_Divulgacao":"canal_divulgacao",
    "Sigla_UF":        "sigla_uf",
}
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    str_cols = ["cnpj_companhia", "nome_empresarial", "canal_divulgacao", "sigla_uf"]
    for col in str_cols:
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_canal_divulgacao ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_canal_divulgacao WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_canal_divulgacao WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_canal_divulgacao ===")
