...
# FDR calculado diretamente — não depende de vw_first_day_return
# Busca cotação do D+1 após o IPO para cada ticker
Q_COTACOES = """
SELECT ticker, data, adj_close, close
...
Q_TICKER_CNPJ = """
SELECT
    REGEXP_REPLACE(v.cnpj_companhia,'[^0-9]','','g')  AS cnpj14,
    v.codigo_negociacao                                AS ticker
FROM cvm_data.cad_valor_mobiliario v
WHERE v.codigo_negociacao IS NOT NULL
...
def main():
    ...
    with engine.connect() as c:
        ...
        cotacoes = pd.read_sql(text(Q_COTACOES),    c)
        ticker_cnpj = pd.read_sql(text(Q_TICKER_CNPJ), c)
    ...
    log.info(f"Cotações carregadas: {len(cotacoes):,} linhas, {cotacoes['ticker'].nunique()} tickers")
    log.info(f"Ticker-CNPJ mapeamento: {len(ticker_cnpj)} registros")
    ...
    # ── Calcular FDR diretamente das cotações ─────────────────────────────
    # 1. Mapear ticker → cnpj14
    ticker_cnpj['cnpj14'] = zfill14(ticker_cnpj['cnpj14'])
    ticker_cnpj = ticker_cnpj.drop_duplicates('cnpj14', keep='first')

    # 2. Preparar cotações
    cotacoes['data'] = pd.to_datetime(cotacoes['data'])
    cotacoes = to_num(cotacoes, ['adj_close','close'])

    # 3. Para cada IPO: encontrar D+1 (primeiro pregão após data_registro_oferta)
    ipo_ticker = ipo_raw.merge(ticker_cnpj, on='cnpj14', how='left')
    ipo_ticker = ipo_ticker.sort_values('data_ipo').drop_duplicates('cnpj14', keep='first')
    ipo_ticker['data_ipo'] = pd.to_datetime(ipo_ticker['data_ipo'])

    fdr_list = []
    for _, row in ipo_ticker.iterrows():
        if pd.isna(row.get('ticker')) or pd.isna(row['data_ipo']):
            continue
        # Preço de oferta: filtro de sanidade (ações BR: R$0.50-R$500)
        preco = row.get('offer_price')
        if pd.isna(preco) or not (0.5 <= float(preco) <= 500):
            continue
        # Cotações do ticker após o IPO
        c_tick = cotacoes[(cotacoes['ticker'] == row['ticker']) &
                          (cotacoes['data']   >  row['data_ipo']) &
                          (cotacoes['data']   <= row['data_ipo'] + pd.Timedelta(days=15))]
        if c_tick.empty:
            continue
        d1 = c_tick.loc[c_tick['data'].idxmin()]
        adj = d1['adj_close']
        if pd.isna(adj) or adj <= 0:
            adj = d1['close']
        if pd.isna(adj) or adj <= 0:
            continue
        fdr_val = (adj - float(preco)) / float(preco)
        fdr_list.append({'cnpj14': row['cnpj14'], 'first_day_return': fdr_val})

    fdr = pd.DataFrame(fdr_list) if fdr_list else pd.DataFrame(columns=['cnpj14','first_day_return'])
    log.info(f"FDR calculado para {len(fdr)} empresas")
