"""
Script : load_dfp_parecer.py
Tabela : cvm_data.dfp_parecer
Fonte  : dfp_cia_aberta_parecer_<ANO>.csv

Fluxo:
  1. Lê o CSV completo (descarta TP_PARECER_DECL como coluna de insert)
  2. Lookup de CNPJs → aborta se houver divergências com cad_cia_aberta
  3. Lookup de tipos de parecer → mapeia TP_PARECER_DECL do CSV para
     id_dfp_parecer_aux + slug_parecer via tabela dfp_parecer_aux
  4. Agregação de texto: para cada grupo (cnpj_cia + dt_refer + versao +
     tp_parecer_decl), concatena TXT_PARECER_DECL ordenado por
     NUM_ITEM_PARECER_DECL em um único campo TEXT (reconstrução semântica)
  5. UPSERT em lotes — sem chave UNIQUE na tabela, mas evita duplicação
     de reprocessamento via year_already_loaded

Motivação da agregação (string_agg):
  O sistema legado da CVM (SQL Server) quebra textos longos (p. ex., o
  Relatório do Auditor Independente) em múltiplos registros sequenciais
  numerados por NUM_ITEM_PARECER_DECL. Reconstruir antes do INSERT garante:
    a) Integridade semântica — palavras não são cortadas entre linhas
    b) Performance em NLP — lematização e stopwords em 1 campo longo é
       mais eficiente que N fragmentos pequenos
    c) Rigor metodológico — descritível no paper como reconstrução via
       agregação de strings antes das técnicas de NLP
...
def insert_batches(conn, df: pd.DataFrame, cad_map: dict) -> None:
    ...
    with conn.cursor() as cur:
        for start in range(0, total, BATCH_SIZE):
            ...
            records = [
                tuple(
                    None if (v is None or v is pd.NaT or (isinstance(v, float) and pd.isna(v))) else
