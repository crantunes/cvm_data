"""
Script: fca_cia_aberta_endereco.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2026\fca_cia_aberta_endereco_2026.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_endereco.log", encoding="utf-8"),
    ],
)
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "tipo_endereco", "logradouro",
        "complemento", "bairro", "cidade", "sigla_uf", "pais",
        "cep", "caixa_postal", "ddi_telefone", "ddd_telefone", "telefone",
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_endereco ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_endereco WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_endereco WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_endereco ===")
