"""
Script: fca_cia_aberta_auditor.py
...
CSV_PATH      = r"D:\DATACVM\Formulário Cadastral (FCA)\fca_cia_aberta_2026\fca_cia_aberta_auditor_2026.csv"
CSV_SEPARATOR = ";"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("load_fca_cia_aberta_auditor.log", encoding="utf-8"),
    ],
)
...
COLUMN_MAP = {
    ...
    "Auditor":                                   "auditor",
    "CPF_CNPJ_Auditor":                          "cpf_cnpj_auditor",
    "Codigo_CVM_Auditor":                        "codigo_cvm_auditor",
    "Origem_Auditor":                            "origem_auditor",
    "Data_Inicio_Atuacao_Auditor":               "data_inicio_atuacao_auditor",
    "Data_Fim_Atuacao_Auditor":                  "data_fim_atuacao_auditor",
    "Responsavel_Tecnico":                       "responsavel_tecnico",
    "CPF_Responsavel_Tecnico":                   "cpf_responsavel_tecnico",
    "Data_Inicio_Atuacao_Responsavel_Tecnico":   "data_inicio_atuacao_responsavel_tecnico",
    "Data_Fim_Atuacao_Responsavel_Tecnico":      "data_fim_atuacao_responsavel_tecnico",
}
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    str_cols = [
        "cnpj_companhia", "nome_empresarial", "auditor",
        "cpf_cnpj_auditor", "codigo_cvm_auditor", "origem_auditor",
        "responsavel_tecnico", "cpf_responsavel_tecnico",
    ]
    for col in str_cols:
        df[col] = df[col].str.strip().replace("", None)

    # Remove linhas sem cnpj_companhia ou id_documento (campos mínimos obrigatórios)
    before = len(df)
...
def build_insert_query() -> str:
    ...
    query = f"""
        INSERT INTO cvm_data.fca_cia_aberta_auditor ({cols_str})
        VALUES %s;
    """
    return query
...
def year_already_loaded(conn) -> bool:
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT COUNT(1) FROM cvm_data.fca_cia_aberta_auditor WHERE fca_ano = %s",
            (FCA_ANO,)
        )
    ...
    if count > 0:
        log.warning(
            ...
            f"DELETE FROM cvm_data.fca_cia_aberta_auditor WHERE fca_ano = {FCA_ANO};"
        )
        return True
    return False
...
def main():
    log.info("=== Início da carga: fca_cia_aberta_auditor ===")
